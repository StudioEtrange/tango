...
__traefik_api_url() {
    ...
    TRAEFIK_API_URL="${TRAEFIK_API_URI_DEFAULT_SECURE}/api"
}
__traefik_api_url
...
__traefik_api_rest_request() {
    ...
    [ "${__http_command}" = "" ] && __http_command="PUT"
     ...
     case $__http_command in
        GET )
            ...
            __tango_log "WARN" "traefik" "traefik_api_rest_request : GET method not supported for rest endpoint $TRAEFIK_API_URL/providers/rest"
    ...
    __tango_log "DEBUG" "traefik" "traefik_api_rest_request PUT REQUEST"
    [ "${__body}" = "" ] && __tango_log "DEBUG" "traefik" "curl -u \"${TRAEFIK_API_USER}\":\"${TRAEFIK_API_PASSWORD}\" -X ${__http_command} -skL \"${__url}\"" \
        || __tango_log "DEBUG" "traefik" "curl -d \"${__body}\" -u \"${TRAEFIK_API_USER}\":\"${TRAEFIK_API_PASSWORD}\" -X ${__http_command} -skL \"${__url}\""
    ...
    case $__http_command in
        ...
        ;;
        PUT )
            __tango_log "DEBUG" "traefik" "traefik_api_rest_request PUT RESULT : $__result"
        ;;
    esac
